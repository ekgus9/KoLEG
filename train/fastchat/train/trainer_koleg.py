...
class Trainer:
    ...
    def _set_signature_columns_if_needed(self):
        if self._signature_columns is None:
            ...
            self._signature_columns += ['input_ids_loc1','labels_loc1','attention_mask_loc1','input_ids_loc2','labels_loc2','attention_mask_loc2','input_ids_por','labels_por','attention_mask_por']
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        input_ids_loc1=inputs.pop("input_ids_loc1")
        labels_loc1=inputs.pop("labels_loc1")
        attention_mask_loc1=inputs.pop("attention_mask_loc1")
        input_ids_loc2=inputs.pop("input_ids_loc2")
        labels_loc2=inputs.pop("labels_loc2")
        attention_mask_loc2=inputs.pop("attention_mask_loc2")
        input_ids_por=inputs.pop("input_ids_por")
        labels_por=inputs.pop("labels_por")
        attention_mask_por=inputs.pop("attention_mask_por")

        inputs_input_ids = torch.cat([inputs['input_ids'], input_ids_por], dim=0)
        inputs_attention_mask = torch.cat([inputs['attention_mask'], attention_mask_por], dim=0)
        inputs_labels = torch.cat([inputs['labels'], labels_por], dim=0)
        inputs = {'input_ids': inputs_input_ids, 'labels': inputs_labels, 'attention_mask': inputs_attention_mask}
        ...
        inputs_input_ids = torch.cat([input_ids_loc1, input_ids_loc2], dim=0)
        inputs_attention_mask = torch.cat([attention_mask_loc1, attention_mask_loc2], dim=0) 
        inputs_loc = {'input_ids': inputs_input_ids, 'attention_mask': inputs_attention_mask}
        outputs_loc = model(**inputs_loc)
        
        with torch.no_grad():
            kl_outputs_loc = self.original_model(**inputs_loc)

        kl_logits_loc1 = outputs_loc.logits[0, -1, :]
        kl_log_probs_loc1 = torch.nn.functional.log_softmax(kl_logits_loc1)
        kl_logits_loc2 = outputs_loc.logits[1, -1, :]
        kl_log_probs_loc2 = torch.nn.functional.log_softmax(kl_logits_loc2)

        loc1_kl = kl_outputs_loc.logits[0, -1, :]
        loc1_kl = torch.nn.functional.log_softmax(loc1_kl)
        loc2_kl = kl_outputs_loc.logits[1, -1, :]
        loc2_kl = torch.nn.functional.log_softmax(loc2_kl)

        loss_loc1 = torch.nn.functional.kl_div(
            loc1_kl, kl_log_probs_loc1, log_target=True, reduction="batchmean"
        )
        loss_loc2 = torch.nn.functional.kl_div(
            loc2_kl, kl_log_probs_loc2, log_target=True, reduction="batchmean"
        )

        loss += loss_loc1 + loss_loc2
