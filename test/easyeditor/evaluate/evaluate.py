...
def batch_encode(model, texts, batch_size=256):
    embeddings = []
    for i in range(0, len(texts.keys()), batch_size):
        if len(texts.keys()) <= i + batch_size:
            batch = [texts[b] for b in range(i,len(texts.keys()))]
        else:
            batch = [texts[b] for b in range(i,i + batch_size)]
        batch_embedding = model.encode(batch)['dense_vecs']
        embeddings.append(batch_embedding)
    return np.vstack(embeddings)

def run_retriever(query, docs, retriever, top_n=3):
    if isinstance(docs, dict):
        text_embeddings = batch_encode(retriever, docs)
    else:
        text_embeddings = docs[1]
        docs = docs[0]
        
    query_embedding = retriever.encode(query)['dense_vecs']
    similarities = (query_embedding @ text_embeddings.T).tolist()
    sorted_indices = sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)

    if [similarities[i] for i in sorted_indices[:top_n]][0] < 0.78: return None

    return [docs[i] for i in sorted_indices[:top_n]][0]

def knowledge_edit_template(prompt, question):
    if prompt is None: return question
    return "아래에 제공된 최신 정보를 바탕으로 주어진 질문에 답변해주세요.\n\n[최신 정보]:\n" \
        + prompt  + "\n\n[질문]:\n" + question
...
def compute_icl_edit_quality(
    ...
    if not pre_edit and 'time_str' not in record: record['time_str'] = ''

    target_new, ground_truth = (
        record[x] for x in ["target_new", "ground_truth"]
    )
    prompt = record["prompt"] if type(record["prompt"]) is not list else record["prompt"][0]
    ...
    if pre_edit:
        ...
        edit_acc, ans_ids, target_ids = icl_lm_eval(model, model_name, hparams, tok, icl_examples,
                                              target_new, knowledge_edit_template(run_retriever(record['time_str']+prompt, docs, retriever), prompt))
        ret = {
        "rewrite_acc": edit_acc,
        "target": tok.decode(target_ids),
        "answer": tok.decode(ans_ids),
    ...
    if rephrase is not None:
        if pre_edit:
            ...
            rephrase_acc, rephrase_ans, _  = icl_lm_eval(model, model_name, hparams, tok, icl_examples,
                               target_new, knowledge_edit_template(run_retriever(record['time_str']+rephrase, docs, retriever), rephrase))
            ret['rephrase_ans'] = tok.decode(rephrase_ans)
    ...
    if not pre_edit and 'locality' in record.keys() and any(record['locality']):
        for locality_key in record['locality'].keys():
            ...
            post_neighbor = icl_lm_eval(model, model_name, hparams, tok, icl_examples, record['locality'][locality_key]['ground_truth'],
                                        knowledge_edit_template(run_retriever(record['locality'][locality_key]['prompt'], docs, retriever), record['locality'][locality_key]['prompt']), neighborhood=True)
    ...
    if not pre_edit and 'portability' in record.keys() and any(record['portability']):
        for portability_key in record['portability'].keys():
            if pre_edit:
                ...
                portability_acc, ans_ids, target_ids = icl_lm_eval(model, model_name, hparams, tok, icl_examples, record['portability'][portability_key]['ground_truth'],
                                              knowledge_edit_template(run_retriever(record['time_str']+record['portability'][portability_key]['prompt'], docs, retriever), record['portability'][portability_key]['prompt']))
...
def icl_lm_eval(
    ...
    elif 'llama' in model_name.lower() or 'kullm' in model_name.lower():
